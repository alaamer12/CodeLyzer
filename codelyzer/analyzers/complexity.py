...
from codelyzer.config import FileMetrics, ProjectMetrics, MetricProvider, ComplexityLevel, MaintainabilityLevel
...
class ComplexityAnalyzer(MetricProvider):
    ...
    def _calculate_cognitive_complexity(self, content: str, language: str) -> int:
        ...
        for line in lines:
            ...
            if language == "python":
                if stripped.endswith(':') and any(kw in stripped for kw in ['if', 'for', 'while', 'elif', 'else', 'try', 'except', 'def', 'class']):
    ...
    def _calculate_maintainability_index(self, file_metrics: FileMetrics, content: str) -> float:
        ...
        halstead_volume = file_metrics.complexity.halstead_complexity or self._calculate_halstead_complexity(content, file_metrics.language)
    ...
    def _extract_operators_and_operands(self, content: str, language: str) -> Tuple[List[str], List[str]]:
        ...
        if language == "python":
            keyword_operators = ['if', 'else', 'elif', 'for', 'while', 'in', 'not', 'is', 'and', 'or', 'return', 'import', 'from']
        else:  # javascript, typescript
            keyword_operators = ['if', 'else', 'for', 'while', 'switch', 'case', 'return', 'typeof', 'instanceof', 'new', 'delete', 'void']
